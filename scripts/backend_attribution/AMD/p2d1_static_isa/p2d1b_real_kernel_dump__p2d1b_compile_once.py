...
# ================================================================================================
# P2D1B diagnostic compile-only harness
# ================================================================================================
...
print("[P2D1B] diagnostic harness active")

# Use one frozen representative split.
BC = 19
BF = 13
ZMIN = -5.75
ZMAX = -3.0
...
GROUP_SIZE = 32
GROUPS = 4096
N_VALUES = GROUPS * GROUP_SIZE

device = "cuda"

# true packed INT4 payload
packed = torch.zeros(
    N_VALUES // 2,
    dtype=torch.uint8,
    device=device
...
# FP32 metadata baseline
fp32_scale = torch.ones(
    GROUPS,
    dtype=torch.float32,
    device=device
)
...
# 32-bit metadata
identity_words = torch.zeros(
    GROUPS,
    dtype=torch.int32,
    device=device
)

mapped_words = torch.zeros(
    GROUPS,
    dtype=torch.int32,
    device=device
)
...
grid = (triton.cdiv(N_VALUES, 256),)

print("[P2D1B] compiling kernel_fp32")
kernel_fp32[grid](
    packed,
    fp32_scale,
    out,
...
torch.cuda.synchronize()
print("[P2D1B] kernel_fp32 PASS")

print("[P2D1B] compiling kernel_identity")
kernel_identity[grid](
    packed,
    identity_words,
    out,
    N_VALUES,
...
torch.cuda.synchronize()
print("[P2D1B] kernel_identity PASS")

print("[P2D1B] compiling kernel_mapped")
kernel_mapped[grid](
    packed,
    mapped_words,
    out,
...
torch.cuda.synchronize()
print("[P2D1B] kernel_mapped PASS")

print("P2D1B_THREE_KERNEL_COMPILE_PASS=True")
